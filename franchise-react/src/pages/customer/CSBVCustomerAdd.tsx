...
import { GSTCustomer } from "./csbv/GST";
import { PANCustomer } from "./csbv/PAN";
import { IECCustomer } from "./csbv/IEC";
import { LUTCustomer } from "./csbv/LUT";
import { BankCustomer } from "./csbv/Bank";
import { ADCodeCustomer } from "./csbv/ADCode";
  ...
  async function updateCustomerEditedDocs(consignor_id: string) {
    CSBVGetStatus({ customer_id: consignor_id }).then((response) => {
      if (response?.status === 200) {
        ...
        if (iec.reason === "" || iec.reason === null) setIecVerified(true);
        else setIecVerified(false);
